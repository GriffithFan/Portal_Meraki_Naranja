...
import { aliasToKey, buildEquipoOptions, dedupeUsersByName } from "@/utils/equipoUtils";
...
export default function EspacioTareasPage() {
  ...
  const fetchData = useCallback(async (options?: { page?: number; append?: boolean }) => {
    ...
    const [tareasRes, estadosRes, espacioRes, camposRes] = await Promise.all([
    ...
    if (camposRes.ok) {
      ...
      const templateCampos = Array.isArray(nextEspacio?.camposConfig) ? nextEspacio.camposConfig : null;
      const nextColumns = templateCampos
      ...
      setColumns(applySavedColumnConfig(nextColumns));
    }
  ...
  }, [COL_CONFIG_KEY, applySavedColumnConfig, espacioId, filterEquipo, filterEstado, filterPrioridad, filterProvincia, includeSubspaces, quickFilter, serverSearch]);
  ...
  useEffect(() => {
    ...
    const openCode = openTargetRef.current;
    if (!openCode) return;
  ...
  const visibleColumns = useMemo(() => {
    if (Array.isArray(espacio?.camposConfig)) return columns.filter(c => c.visible);
  ...
  const createViewSnapshot = useCallback((name: string, id?: string): TareasSavedView => ({
    ...
    search,
    filters: { filterEstado, filterProvincia, filterEquipo, filterPrioridad, quickFilter, groupBy, includeSubspaces },
    sortConfig,
    columns: columns.map((col, index) => ({ id: col.id, visible: col.visible, order: index, width: col.width })),
    updatedAt: new Date().toISOString(),
  }), [columns, filterEquipo, filterEstado, filterPrioridad, filterProvincia, groupBy, includeSubspaces, quickFilter, search, sortConfig]);
  ...
  function applySavedView(view: TareasSavedView) {
    ...
    setFilterEquipo(filters.filterEquipo || "");
    setFilterPrioridad(filters.filterPrioridad || "todas");
    setQuickFilter(filters.quickFilter || "todos");
    setGroupBy(filters.groupBy || "estado");
    ...
    if (Array.isArray(view.columns) && view.columns.length > 0) {
      setColumns((current) => {
        ...
        return [...ordered, ...current.filter((col) => !used.has(col.id))];
      });
    }
  ...
  const getSavedViewSummary = useCallback((view: TareasSavedView) => {
    const filters = view.filters || {} as TareasSavedView["filters"];
    const count = [filters.filterEstado !== "todos", Boolean(filters.filterProvincia), Boolean(filters.filterEquipo), filters.filterPrioridad !== "todas", filters.quickFilter !== "todos", filters.includeSubspaces === false].filter(Boolean).length;
    ...
    <div className="animate-fade-in-up">
      ...
      {showCreateModal && (
        <CreateTareaModal
          ...
          fieldsConfig={columns.filter((column) => column.visible)}
