...
import { buildEquipoOptions, dedupeUsersByName, getEquipoDisplayName } from "@/utils/equipoUtils";
...
export default function TareasPage() {
  ...
  const createViewSnapshot = useCallback((name: string, id?: string): TareasSavedView => ({
    ...
    search,
    filters: { filterEstado, filterProvincia, filterEquipo, filterPrioridad, quickFilter, groupBy },
    sortConfig,
    columns: columns.map((col, index) => ({ id: col.id, visible: col.visible, order: index, width: col.width })),
    updatedAt: new Date().toISOString(),
  }), [columns, filterEquipo, filterEstado, filterPrioridad, filterProvincia, groupBy, quickFilter, search, sortConfig]);
  ...
  const getSavedViewSummary = useCallback((view: TareasSavedView) => {
    const filters = view.filters || {} as TareasSavedView["filters"];
    const count = [filters.filterEstado !== "todos", Boolean(filters.filterProvincia), Boolean(filters.filterEquipo), filters.filterPrioridad !== "todas", filters.quickFilter !== "todos"].filter(Boolean).length;
