...
const ACCION_CONF: Record<string, { label: string; color: string }> = {
  ...
  CREAR: { label: "Creación", color: "bg-emerald-500" },
  EDITAR: { label: "Edición", color: "bg-amber-500" },
  ELIMINAR: { label: "Eliminación", color: "bg-red-500" },
};

const ACCIONES_POR_FUENTE: Record<Fuente, { value: string; label: string }[]> = {
  cambios: [
    { value: "", label: "Todas" },
  ...
  accesos: [
    ...
    { value: "CONSULTA_PREDIO", label: "Consultas de predio" },
    { value: "CONSULTA_MERAKI", label: "Consultas Meraki" },
  ],
};
...
export default function AuditoriaPage() {
  ...
  const [fuente, setFuente] = useState<Fuente>("cambios");
  ...
  const fetchRegistros = useCallback(
    async (offset = 0, append = false) => {
      ...
      if (fuente === "cambios" && filtroEntidad) params.set("entidad", filtroEntidad);
      ...
      if (res.ok) {
        ...
        if (data.entidades) setEntidades(data.entidades);
      }
      if (!append) setLoading(false);
      else setLoadingMore(false);
    },
    [fuente, filtroAccion, filtroUsuario, filtroEntidad, filtroDesde, filtroHasta]
  );
  ...
  const hayFiltros = filtroAccion || filtroUsuario || filtroEntidad || filtroDesde || filtroHasta;

  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto space-y-5 p-4 sm:p-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-surface-800 dark:text-surface-100">Auditoría</h1>
        <p className="text-sm text-surface-500 mt-1">
          {fuente === "cambios" ? "Todas las modificaciones (creación, edición y borrado) hechas por cada usuario" : "Inicios de sesión y consultas de predios/Meraki"}
          {total > 0 && <span className="ml-2 text-surface-400">({total.toLocaleString("es-AR")} registros)</span>}
        </p>
      </div>

      {/* Pestañas de fuente */}
      <div className="inline-flex bg-surface-100 dark:bg-surface-800 rounded-lg p-1">
        <button onClick={() => cambiarFuente("cambios")}
          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${fuente === "cambios" ? "bg-white dark:bg-surface-700 text-surface-800 dark:text-surface-100 shadow-sm" : "text-surface-500 hover:text-surface-700"}`}>
          Modificaciones
        </button>
        <button onClick={() => cambiarFuente("accesos")}
          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${fuente === "accesos" ? "bg-white dark:bg-surface-700 text-surface-800 dark:text-surface-100 shadow-sm" : "text-surface-500 hover:text-surface-700"}`}>
          Accesos
        </button>
      </div>
          ...
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col gap-1">
              ...
              <select value={filtroAccion} onChange={(e) => setFiltroAccion(e.target.value)} className={inputClass}>
                {ACCIONES_POR_FUENTE[fuente].map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>

            {fuente === "cambios" && (
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-surface-500">Sección / Entidad</label>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select value={filtroUsuario} onChange={(e) => setFiltroUsuario(e.target.value)} className={inputClass}>
                <option value="">Todos</option>
                {usuarios.map((u: any) => <option key={u.id} value={u.id}>{u.nombre} ({u.rol})</option>)}
              </select>
            </div>
            ...
            {hayFiltros && (
              <button
                onClick={() => { setFiltroAccion(""); setFiltroUsuario(""); setFiltroEntidad(""); setFiltroDesde(""); setFiltroHasta(""); }}
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-16 text-surface-400">
              ...
              <p className="text-xs mt-1">{fuente === "cambios" ? "Las modificaciones se registran automáticamente" : "Los accesos se registrarán a partir de ahora"}</p>
            </div>
          ) : (
            <div className="divide-y divide-surface-100 dark:divide-surface-700">
              {registros.map((r: any) => {
                const conf = ACCION_CONF[r.accion] || { label: r.accion, color: "bg-surface-400" };
                const campos: string[] = Array.isArray(r?.metadata?.campos) ? r.metadata.campos : [];
                return (
                  <div key={r.id} className="flex items-start gap-3 px-4 sm:px-5 py-3 sm:py-4 hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-surface-800 dark:text-surface-200 leading-snug">
                        ...
                        {" — "}
                        {fuente === "cambios" ? (r.descripcion || conf.label) : conf.label}
                        {fuente === "accesos" && r.detalle && (
                          <span className="text-surface-500 dark:text-surface-400">{" · "}{r.detalle}</span>
                        )}
                      </p>
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1.5 text-xs text-surface-400">
                        ...
                        {fuente === "cambios" && r.entidad && <span className="font-mono text-surface-500">{r.entidad}</span>}
                        {fuente === "cambios" && campos.length > 0 && <span className="text-surface-400">campos: {campos.join(", ")}</span>}
                        {r.usuario?.rol && <span>{r.usuario.rol}</span>}
                        {r.ip && <span className="font-mono">{r.ip}</span>}
                        <span>{format(new Date(r.createdAt), "dd MMM yyyy, HH:mm:ss", { locale: es })}</span>
                      </div>
                    </div>
                  </div>
                );
              ...
              {hasMore && (
                <div className="text-center py-4">
                  <button onClick={() => fetchRegistros(registros.length, true)} disabled={loadingMore}
