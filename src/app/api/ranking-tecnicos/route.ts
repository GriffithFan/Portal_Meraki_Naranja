...
export async function GET() {
  ...
  for (const predio of predios) {
    ...
    const assignedUsers = predio.asignaciones
      ...
      .filter((usuario) => usuario?.activo && usuario.rol === "TECNICO");

    const targets = assignedUsers.length > 0
      ? assignedUsers.map((usuario) => ({
          id: usuario.id,
          nombre: usuario.nombre,
          equipoKey: resolveEquipoKey(usuario.nombre) || usuario.nombre,
        }))
      : (() => {
          const equipoKey = predio.equipoAsignado ? resolveEquipoKey(predio.equipoAsignado) || predio.equipoAsignado : null;
          return equipoKey ? [{ id: equipoKey, nombre: getEquipoDisplayName(equipoKey), equipoKey }] : [];
        })();

    for (const target of targets) {
      const current = ranking.get(target.id) || {
        tecnicoId: target.id,
        tecnicoNombre: getEquipoDisplayName(target.nombre),
